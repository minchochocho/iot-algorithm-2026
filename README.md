...
- 정의
    - 데이터를 구성하는 구조. 데이터를 원하는 요구에 따라 효율적으로 처리하기 위해 사용
    - 자료(Data) + 구조(Structure) -> 자료구조(DataStructure)
...
- 종류 - 자료구조를 가지고 해결하는 방법
    - `검색` : 특정 데이터 찾기. 선형 검색, 이진 탐색
    - `재귀` 알고리즘
    - `정렬` : 삽입, 선택, 버블, 쉘, 퀵
    - `탐색` : 모든 데이터 방문. DFS(깊이 우선 탐색), BFS(너비 우선 탑색), 이진 탐색
...
2. 메모리 구조
    - 코드 영역 - 소스코드가 저장. 컴파일 시 할당
    - 데이터 영역 - 전역, 정적 변수 할당. 프로그램 시작 시 할당, 종료 시 메모리 해제
    - 스택 영역 - 함수 호출 시 생성되는 지역변수, 매개변수 저장, 함수 호출 완료 시 메모리 해제
    - `힙 영역` - 동적으로 메모리를 할당. malloc, calloc, realloc와 연계
...
3. 이진 검색 : [소스](./basic/algorithm02/app02/app02.c)
    - 데이터가 키 값으로 이미 정렬(sort)되어 있다는 가정하에서 사용 - 단점
...
4. `복잡도(Complexity)`
    ...
    - 선형 검색 시간 복잡도 : $O(n)$
    - 이진 검색 시간 복잠도 : $O(log n)$
    - 시간복잡도 일반적 사용
        
        |시간복잡도|의미|비고|
        |:--|:--|:--|
        |$O(1)$| 상수 시간| 입력크기와 관계없이 항상 일정 횟수 실행 |
        ...
        |$O(n^3)$| 삼차 시간| 삼중 반복문 |
        |$O(nk)$| 선형x 다른변수| n과 k가 독립변수일때, 문자열 비교 n개 x 길이 k, 문자열 배열 처리... |
...
2. 기술
    - `기초프로그래밍`, `알고리즘`, 통신프로그램, 데이터 분석, 웹 개발, 윈앱 개발, IoT
...
2. 큐 : [소스](./basic/algorithm03/app01/app01.c)
    - 양쪽이 다 열려있어서 한쪽에서 데이터를 추가, 반대에서 꺼내는 자료구조
    - 실생활 예, 영화관 티켓줄, 버스 대기줄, 식장 대기줄,...
    - 컴퓨터 예, 파일입출력 스트림, 프린터 출력대기(Pool), 키보드 입력 순서버퍼,... 
...
1. 의미 - Recursive(재귀)
    ...
    - 함수(알고리즘)이 자기 자신의 함수를 호출하여, 더 작은 하위 문제를 해결하는 프로그램 기법
...
5. `메모이제이션(memoization)` [소스](./basic/algorithm04/app01/app01.c)
    ...
    - 동적계획법에 많이 사용
    - 단점 - 메모리를 매우 많이 사용. 프로그램이 비정상 종료될 수 있음
...
1. 데이터를 기준값에 따라 오름차순(작은값부터 큰값으로), 내림차순(큰값부터 작은값으로)
...
3. 선택정렬
    - 가장 작은 요소부터 선택해서 알맞은 위치에 옮겨서 순서대로 정렬(찍어서 교체)
    - 가장 작은 값을 앞으로 보내는 알고리즘
    - 시간복잡도 : 모두 $O(n^2)$

4. 삽입정렬
    - 이미 정렬된 부분에 새 원소를 끼워넣는 정렬방식(카드정리)
    - 시간복잡도 : 평균 $O(n^2)$, 최악 $O(n^2)$, $O(n)$

5. 셸정렬
    - 삽입정렬을 멀리 떨어진 간격으로 먼저 수행한 뒤  점점 간격을 줄여가는 정렬방식
    - 삽입정렬을 업그레이드해서 멀리 있는 값부터 정리
    - 삽입정렬이 완전 뒤죽박죽일때 시간복잡도가 좋지 않음
    - 시간복잡도 : 평균 $O(n^{1.3})$ ~ 평균 $O(n^{1.5})$, 최악 $O(n^2)$, 최선 $O(n log n)$

6. 퀵정렬
    - 기준값을 정하고, 작은값은 왼쪽, 큰값은 오른쪽으로 나눠서 재귀적으로 정렬하는 방식
...
1. 부루트 포스 - 모든 문자가 전부 일치하는지 전부 비교하는 방법
    - 횟수제한 등으로 막힘

2. KMP - 부루트-포스 방법에서 패턴 형태로 변경된 비교 방법

3. 보이어-무어 - 부르트-포스와 KMP 상에서 옮길 크기를 정해서 검색하는 방법
...
2. 연결 리스트 - 포인터로 각 노드가 연결된 형태
    - 배열과 달리 중간에 데이터가 빠지거나 추가되어도 배열처럼 데이터를 옮기는 추가 처리 필요없음
...
1. 검색과 데이터 추가, 삭제가 효율적으로 사용할 수 있는 자료구조
    ...
    - 자료 접근(검색)이 $I(1)$ 정도로 매우 빠름
    - 해시 함수를 통해서 만들어진 key와 실제 값(value)을 같이 저장

2. 동일한 Key가 만들어져 충돌 가능하다는 단점이 존재
